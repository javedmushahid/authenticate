...
const Dashboard = () => {
  const {
    ...
    error,
    mutate,
  } = useSWR("http://localhost:8080/get-all-tickets", fetcher);
  ...
  const handleUpdatePassenger = async (updatedDetails) => {
    try {
      await axios.put(
        `http://localhost:8080/update-ticket/${updatedDetails._id}`,
        updatedDetails
      );
  ...
  const handleDeletePassenger = async (passengerToDelete) => {
    try {
      await axios.delete(
        `http://localhost:8080/delete-ticket/${passengerToDelete._id}`
      );
      // Refresh the data by re-fetching
      mutate();
    } catch (error) {
